...
if(command ==="suggest"){
  ...
  if(args.length>=2){
	...
	const amountSubmitted = sql.prepare("SELECT * FROM questions WHERE user = ?").all(message.member.id);
	...
	message.channel.send("You have already made 5 suggestions, you can't submit more until yours are used.");
	}		
  } else{
...
if(command === "waitlist"){
  ...
  const notApproved = sql.prepare("SELECT * FROM questions WHERE approved = 0 ORDER BY date LIMIT 11").all();
  console.log("Waitlist query generated");
  const questionsEmbed = new Discord.RichEmbed()
  .setTitle("Unapproved QOTDs")
  .setAuthor(client.user.username,client.user.avatarURL)
  .setDescription("Approve or deny QOTDs by typing q!approve/deny id. If you deny a QOTD it will be deleted from the waitlist. Please don't delete the first one, it is what makes the waitlist work.")
  .setColor("RANDOM");
  ...
  for(const data of notApproved){
	...
	questionsEmbed.addField(client.user.username, "There are no pending questions!");
	break;
	}
  }
...
function randomQuestion(){
    ...
    sql.prepare("DELETE FROM questions WHERE id =?;").run(rq.id);
      ...
      client.channels.get("608080775413235755").send("There was no question to send today! Suggest more!");
    }
}
